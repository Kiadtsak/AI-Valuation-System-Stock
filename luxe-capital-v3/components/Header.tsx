  ...
  return (
    <nav className="hidden md:flex items-center gap-1">
      {NAV.map((item) => {
        ...
        // Preserve symbol param when switching between analysis pages
        const href = item.href === '/' || !symbol ? item.href : `${item.href}?symbol=${symbol}`;
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        {/* Nav — Suspense needed because useSearchParams requires it */}
        <Suspense fallback={<div className="hidden md:block w-96" />}>
          <NavItems />
        </Suspense>
