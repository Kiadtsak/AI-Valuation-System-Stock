...
import {
  ...
  FileText, Brain, Zap, TrendingUp, ShieldCheck, ArrowLeft,
} from 'lucide-react';
...
function AIAnalysisContent() {
  ...
  const handleSearch = async (s: string) => {
    ...
    setAnalysis('');
    setSymbol(s);
    window.history.replaceState(null, '', `/ai-analysis?symbol=${s}`);
    try {
      ...
      <main className="relative max-w-7xl mx-auto px-6 pt-10 pb-24">
        ...
        <div className="grid lg:grid-cols-[380px_1fr] gap-6 items-start">
          ...
          >
              ...
              <div className="glass rounded-3xl p-6">
                ...
                <div className="space-y-2.5">
                  ...
                  {data?.valuation?.intrinsic_value_per_share ? (
                    <MiniRow label="DCF Intrinsic" value={`$${data.valuation.intrinsic_value_per_share.toFixed(2)}`} highlight />
                  ) : null}
                </div>
              </div>
            )}
            ...
            <div className="glass rounded-3xl p-6">
              <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--text-muted)] mb-4">Engine</div>
              ...
              <div className="relative flex items-center justify-center gap-3">
                {running ? (
                  <>
                    ...
                    <span className="font-mono text-xs uppercase tracking-[0.2em] font-bold">Analyzing...</span>
                  </>
                ) : (
                  <>
                    <Sparkles size={16} />
          ...
          >
            ...
            <div className="relative border-b border-[var(--border-subtle)] px-8 py-5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                {analysis && !running && (
                  ...
                  >
                    {copied ? (
                      <><CheckCircle2 size={10} className="text-[var(--positive)]" />Copied</>
                    ) : (
                      <><Copy size={10} />Copy</>
                    )}
                  </button>
            ...
            <div ref={outputRef} className="relative p-8 md:p-12 max-h-[800px] overflow-y-auto">
              <AnimatePresence mode="wait">
                {!analysis && !running ? (
                  ...
                  <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    {[...Array(6)].map((_, i) => (
                      <div key={i} className="h-4 bg-[var(--bg-elevated)] rounded shimmer" style={{ width: `${60 + Math.random() * 40}%` }} />
                    ))}
                  </motion.div>
                ) : (
...
function MiniRow({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-baseline justify-between">
      ...
      <span className={cn('num-display text-sm', highlight ? 'text-gold font-semibold' : 'text-[var(--text-primary)]')}>
        {value}
      </span>
    </div>
  );
