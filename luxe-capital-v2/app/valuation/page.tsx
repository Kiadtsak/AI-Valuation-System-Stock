...
import {
  ...
  ArrowUpRight, ArrowDownRight, Gauge, AlertTriangle, CheckCircle2,
} from 'lucide-react';
...
export default function ValuationPage() {
  ...
  // DCF state
  const [initGrowth, setInitGrowth] = useState(0.12);     // 12%
  const [termGrowth, setTermGrowth] = useState(0.03);     // 3%
  ...
  const handleSearch = async (s: string) => {
    ...
    try {
      ...
      // Derive initial growth from historical FCF CAGR
      ...
      if (fcf0 && fcfN && fcf0 > 0 && fcfN > 0 && rows.length > 1) {
        ...
        const clipped = Math.max(-0.1, Math.min(0.25, cagr));
        setInitGrowth(clipped);
      }
    } finally {
      setLoading(false);
    }
  ...
  // Derive baseFCF, shares, netCash, price from latest data
  const baseInputs = useMemo(() => {
    if (!data) {
      return { baseFCF: 100e9, shares: 15e9, netCash: 0, currentPrice: 175 };
    }
          ...
          <div className="grid lg:grid-cols-[420px_1fr] gap-6 items-start">
            ...
            >
              ...
              <div className="space-y-7">
                <LuxeSlider
                  label="Initial FCF Growth"
                  value={initGrowth}
                  min={-0.1} max={0.4} step={0.005}
                  format={(v) => fmtPct(v * 100, 1)}
                  onChange={setInitGrowth}
                />
                <LuxeSlider
                  label="Terminal Growth"
                  value={termGrowth}
                  min={0} max={0.05} step={0.001}
                  format={(v) => fmtPct(v * 100, 2)}
                  onChange={setTermGrowth}
                />
                <LuxeSlider
                  label="WACC (Discount Rate)"
                  value={wacc}
                  min={0.03} max={0.20} step={0.002}
                  format={(v) => fmtPct(v * 100, 2)}
                  onChange={setWacc}
                />
                <LuxeSlider
            ...
            <div className="space-y-6">
              ...
              <motion.div
                ...
                className={cn(
                  'relative glass rounded-3xl p-8 md:p-10 overflow-hidden',
                  'bg-gradient-to-br',
                  V.bg
                )}
              >
                ...
                <div className="grid md:grid-cols-2 gap-8 items-center">
                  <div>
                    ...
                    <div className="mt-5 flex items-center gap-3 flex-wrap">
                      <div className={cn(
                        ...
                        {result.upside >= 0 ? (
                          <ArrowUpRight size={12} className="text-[var(--positive)]" />
                        ) : (
                          <ArrowDownRight size={12} className="text-[var(--negative)]" />
                        )}
                        <span className={cn(
                          'num-display text-xs font-semibold',
                          result.upside >= 0 ? 'text-[var(--positive)]' : 'text-[var(--negative)]'
                        )}>
                          {result.upside >= 0 ? '+' : ''}{result.upside.toFixed(1)}%
                        </span>
                      </div>
                      <span className="text-xs text-[var(--text-muted)]">
                        vs current ${baseInputs.currentPrice.toFixed(2)}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <ValueBar
                      label="Explicit Forecast PV"
                      value={result.sumPVExplicit}
                      total={result.enterpriseValue}
                    />
                    <ValueBar
                      label="Terminal Value PV"
                      value={result.pvTerminal}
                      total={result.enterpriseValue}
                    />
                    ...
                    <div className="pt-3 border-t border-[var(--border-subtle)]">
                      <div className="flex items-baseline justify-between">
                        ...
                        <span className="num-display text-xl text-[var(--text-primary)]">
                          {fmtMoney(result.enterpriseValue)}
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              >
                <div className="flex items-baseline justify-between mb-6">
                  <div>
                    <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--accent)] mb-1">
                      ∕ Forecast
                    </div>
              ...
              >
                ...
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr>
                        <th className="text-left p-3 font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--text-muted)]">
                          WACC \ g
                        </th>
                        {tgRange.map((tg) => (
                          <th
                            key={tg}
                            className="text-right p-3 font-mono text-[10px] uppercase tracking-[0.2em] text-[var(--text-muted)]"
                          >
                            {fmtPct(tg * 100, 1)}
                          </th>
                        ))}
                ...
                <div className="mt-4 flex items-center gap-4 text-[10px] font-mono text-[var(--text-muted)] uppercase tracking-widest">
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 bg-[var(--positive)]/40 rounded-sm" /> &gt; 15% upside
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 bg-[var(--negative)]/40 rounded-sm" /> &gt; 15% downside
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-2 h-2 ring-2 ring-[var(--accent)] rounded-sm" /> current assumption
                  </span>
                </div>
              </motion.div>
            </div>
